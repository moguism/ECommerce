    ...
    public class EmailService
    {
        ...
        public async Task CreateEmailUser(User user, Wishlist productsorder,int paymetId)
        {
            ...
            foreach (ProductsToBuy products in productsorder.Products)
            {
                ...
                body.AppendLine($"<tr style='text-align: center;'><td>{oneproduct.Name}</td><td style='width: 100%; max-width: 50px; height: 200px; border-radius: 25px;'><img src='https://farminhouse.runasp.net{oneproduct.Image}'></td><td>{price}€</td><td>{products.Quantity}</td><td>{totalpricequantity}€</td></tr>");
